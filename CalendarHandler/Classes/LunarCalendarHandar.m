...
@implementation LunarCalendarHandar

- (NSArray<DateModel *> *)getLunarCalendarDataAtYear:(NSInteger)year {
    if (year <= 1990) {
        NSLog(@"抱歉，暂不提供1990年及之前的数据！");
        return nil;
    } else if (year >= 2100) {
        NSLog(@"抱歉，暂不提供2100年及之后的数据！");
        return nil;
    } else {
        return [self getLunarCalendarDataAtYearData:LunarCalendarTable[year - 1901]];
    }
}

- (NSArray <DateModel *>*)getLunarCalendarDataAtYearData:(NSInteger)data {
    //
    NSMutableArray <DateModel *>* lunarCalendarData = [NSMutableArray new];
